...
export default function Home() {
  ...
  function handlePrint() {
    ...
    localStorage.setItem('tabela_precos_print_config', JSON.stringify({
  ...
  function removeProduct(cod) {
    saveProducts(products.filter(p => p.cod !== cod))
  }
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '0 0 60px' }}>
      {/* ── Header ── */}
      <header style={{ background: 'linear-gradient(135deg, #1b4332 0%, #2d6a4f 60%, #40916c 100%)', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 12px rgba(27,67,50,0.25)' }}>
        ...
        <div>
          <h1 style={{ color: 'white', fontSize: 16, fontWeight: 700, margin: 0 }}>Tabela de Preços</h1>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '18px 18px 0' }}>
        ...
        <div className="section-card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: '#2d6a4f', minWidth: 90 }}>Tipo de Venda</label>
        ...
        <div className="section-card">
          <div className="section-title" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, color: '#6b9e7a', fontWeight: 400 }}>
              ...
              <button
                ...
                title="Gerar PDF da tabela de preços"
              >
                🖨 Imprimir PDF
              </button>
            </div>
          ...
          <div style={{ padding: '12px 14px', borderBottom: '1px solid #e8f2ea', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <button className="btn-primary" style={{ background: 'linear-gradient(135deg, #52b788, #40916c)' }} onClick={() => setShowModal(true)}>
              🔍 Pesquisar produto
            </button>
            {barcodeError && (
          ...
          <div className="product-grid-container">
            {products.length === 0 ? (
              <div className="empty-state" style={{ padding: '48px 16px' }}>
                ...
                <div style={{ fontWeight: 600, color: '#5a8a65', marginBottom: 4 }}>Nenhum produto adicionado</div>
                ...
                <tbody>
                  {products.map((p, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '8px 12px', fontWeight: 600, color: '#2d6a4f' }}>{p.cod}</td>
          ...
          {products.length > 0 && (
            <div style={{ padding: '8px 14px', borderTop: '1px solid #e8f2ea', fontSize: 11, color: '#6b9e7a', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>⠿</span>
      ...
      {(selectedConds.length > 0 || selectedDeadlines.length > 0 || selectedSaleType || products.length > 0) && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'linear-gradient(135deg, #1b4332, #2d6a4f)', padding: '10px 24px', display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center', boxShadow: '0 -2px 12px rgba(0,0,0,0.15)', zIndex: 100 }}>
          ...
          {selectedConds.length > 0 && (
            <span style={{ color: 'white', fontSize: 11 }}>
              <span style={{ color: '#95d5b2' }}>Condições:</span> {selectedConds.length} selecionada{selectedConds.length !== 1 ? 's' : ''}
            </span>
          )}
          {selectedDeadlines.length > 0 && (
            <span style={{ color: 'white', fontSize: 11 }}>
              <span style={{ color: '#95d5b2' }}>Prazos:</span> {selectedDeadlines.length} selecionado{selectedDeadlines.length !== 1 ? 's' : ''}
            </span>
          )}
          {selectedSaleType && (
            <span style={{ color: 'white', fontSize: 11 }}>
              <span style={{ color: '#95d5b2' }}>Tipo Venda:</span>{' '}
              {trimStr(saleTypes.find(s => trimStr(s.Z01_CODIGO) === selectedSaleType)?.Z01_DESCRI || selectedSaleType)}
            </span>
          )}
          {products.length > 0 && (
            <span style={{ color: 'white', fontSize: 11 }}>
              <span style={{ color: '#95d5b2' }}>Produtos:</span> {products.length}
            </span>
          )}
        </div>
      )}
